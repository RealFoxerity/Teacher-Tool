...
Currently working on .bat launched script with a small window because malwarebytes and similar are disabling vbs...  
Also adding a new file to add scripts instead of current copy methods.   
And as an example for that feature i am also making a copy script to work with a list file, so that you know what files were copied.  
Side effect of this will be, that you can have only the list instead of the files, so next time you plug in the usb, you dont need the files to still be present...
